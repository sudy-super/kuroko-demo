...
/* Task 10f — 縁のレンズ。Apple の参考画像 (Liquid Glass の紹介、2025-06-09) では、
   ...
   - edgeReach: 帯の中で下の内容をどれだけ引き寄せるか。実寸は「短辺の半分 x 2 x 1.24 x この値」
     画素で、上部バーなら約 9.5px。帯の幅より小さくしてある。0.58 まで上げたときは、
     引き寄せる量が帯の幅の 3 倍を超えて縁のずっと外や内から拾うようになり、帯の中身が
     元の像と繋がらない白い筋になった。帯の中で像が切れずに引き伸びるのは、
     引き寄せる量が帯の幅を超えないところまで。refraction も同じ理由で上限の 110 から下げた
...
/* Task 10f — 段は 1 つだけ。iOS 26 でガラスになるのはナビ層 (バー、ボタン、ツールバー) で、
   ...
   tint はライブラリの 0〜1.5 の目盛り。0.06 は参考画像の「ほぼ素通し」に合わせた値。
   0.14 と下ぼかし 3 を入れていたときは、面の中が一様な乳白色になって下の像が消え、
   縁が一律の面取りにしか見えなかった (ユーザー判定)。下ぼかしは 0 にした。
   背後が濃くて文字が 4.5:1 を割る面 (サイドナビと Today のボタン列)だけ、
   その場で tint を上書きする。段は増やさない */
