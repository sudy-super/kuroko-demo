...
export interface Suggestion { id: string; source: 'chat' | 'transcript' | 'email' | 'line'; kind: SuggestionKind; status: 'pending' | 'accepted' | 'rejected'; reason: string; payload: SuggestionPayload; createdAt: string }
...
export interface Db { version: number; seededOn: string; user: User; people: Person[]; identities: ChannelIdentity[]; companies: Company[]; projects: Project[]; threads: MessageThread[]; messages: Message[]; events: CalendarEvent[]; meetings: Meeting[]; transcripts: Transcript[]; tasks: Task[]; /** ToDo を自分で並べた順 (id の並び)。無ければ期限順 (task-reorder.md) */ taskOrder?: string[]; documents: Document[]; approvals: Approval[]; suggestions: Suggestion[]; scheduling: SchedulingRequest[]; logs: ActivityLog[]; chat: ChatMessage[]; line: LineMessage[]; slack: LineMessage[]; settings: Settings; demo: Demo }
