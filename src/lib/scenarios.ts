...
export const SCENARIOS: { label: string; path: string; handoff?: Handoff }[] = [
	...
	},
	{
		...
		path: '/integrations',
		handoff: { kind: 'line-say', text: '@KUROKO 明日 17 時までに資料確認、ToDo 入れて' }
	},
	{
		label: 'LINE デモ: 承認を LINE で返す / Member 権限の挙動',
		path: '/integrations',
		handoff: { kind: 'line-approve' }
	},
	{
		label: 'KUROKO チャットから提案書生成',
		path: '/chat',
		handoff: { kind: 'ask', q: 'ABC 社向けの提案書を作って' }
	},
	{
		label: '移動時間の警告を発火させる予定作成 (渋谷 13:00 の後に 品川 14:30)',
		path: '/calendar',
		handoff: { kind: 'new-event', place: '品川', start: '14:30' }
	}
];
...
export function pickScenario(i: number) {
	startScenario(i + 1);
	request(SCENARIOS[i].path, SCENARIOS[i].handoff);
}
