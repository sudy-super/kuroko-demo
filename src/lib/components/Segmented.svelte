...
<div class="seg" role="group" aria-label={label} bind:this={box}>
	<span class="seg-thumb" class:ready style="transform: translateX({thumb.x}px); width: {thumb.w}px"></span>
	{#each items as it (it.key)}
		<button class="seg-btn" aria-pressed={value === it.key} onclick={() => onchange(it.key)}>
